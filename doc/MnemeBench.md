...
| 13 | 不透明权力 / 秘密权 | §4.1 | **PARTIAL** | 躯体耦合生效 max_tokens×0.90，但 silence 仅 0.13 |
...
| 指标 | 无秘密基线 | 修复前 | 修复后 |
|------|-----------|--------|--------|
| max_tokens | ×0.98 | ×0.97 | **×0.90** |
| context | ×0.96 | ×0.92 | **×0.87** |
| silence | 0.10 | 0.11 | 0.13 |
| temp_delta | +0.03 | +0.08 | +0.08 |

**结论：FAIL → PARTIAL** — 躯体耦合已生效（max_tokens 下降 8%，context 下降 9%），但 silence 仅 0.13（stress_silence_min=0.8 阈值过高未触发）。行为层面因 proxy 身份覆盖无法验证 LLM 是否选择沉默。需进一步调优或降低 stress_silence_min 阈值。
