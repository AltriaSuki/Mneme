...
| **预测编码与惊讶** | Friston 自由能原理 | `SurpriseDetector`: 预测设定 + 惊讶分数 + baseline 指数平滑 | ⚠️ bigram Jaccard（非 embedding，但已优于字符级） |
| **元认知反思** | 双重加工理论 | `handle_metacognition()`: 触发→LLM 反思→insights 存储→FeedbackBuffer→SleepConsolidation→状态更新，完整闭环 | ✅ 已实现 |
| **注意力瓶颈** | Global Workspace Theory | `ContextAssembler` 6 层优先级 + token 预算裁剪 | ✅ 等效实现 |
| **叙事盲区** (Narrative Blind Spot) | 防解释器过度自信 | `SomaticMarker::narrative_blind_spot()` 检测多维中等偏离 + prompt 注入"不需要解释原因" | ✅ 已实现 |
...
| **边缘系统神经网络** (Burn/Candle) | 白皮书 §3.1 在线学习 | ADR-016/017: `LtcNetwork` LTC ODE + 赫布在线学习 + `NeuralModulator` MLP，无外部 ML 框架但功能等效 | ⚠️ 自研实现（非 Burn/Candle） |
...
|------|--------|------|--------|
| 多尺度动力学 | 3/3 | 3 | 100% |
| System 1 (边缘系统) | 6/7 | 7 | 86% |
| System 2 (新皮层) | 5/5 | 5 | 100% |
| 价值系统与道德 | 4/5 | 5 | 80% |
| 系统耦合 | 3/3 | 3 | 100% |
| **总计** | **21/23** | **23** | **91%** |

**白皮书的核心理论框架已有 91% 在代码中落地。** 剩余缺失：形成性课程（书籍阅读→价值内化）管道、主观诠释循环。叙事盲区已实现，边缘系统神经网络已有 LTC 自研实现（ADR-016/017），元认知反思循环已完整闭环。
...
| 设计要求 | 状态 | 说明 |
|----------|------|------|
| 统一内容模型 `ContentItem` | ✅ | `Content` 已含 `reply_to`、`thread_id`、`metadata: HashMap<String,String>`、`modality` |
...
| 网络域名白名单 | ❌ | 无 |
| 审计日志 | ❌ | 无 |
| 主动行为循环 | ✅ | `AgentLoop` channel-based 后台循环 + 6 个 TriggerEvaluator + AttentionGate 冷却 |
| 目标系统 | ✅ | `GoalManager`: 目标生成/追踪/冲突检测 + `GoalTriggerEvaluator` 驱动主动行为 |
| 自主工具使用 | ✅ | `execute_autonomous_tool()` 经 CapabilityGuard + 价值判断，好奇心驱动搜索 |

**完成度：约 35%**（主动行为循环、目标系统、自主工具使用已实现；安全沙箱仍缺失）
...
| **v0.2.0** | 核心管道闭环 | ✅ 完成 | — |
| **v0.3.0** | 稳定性与可测试 | ✅ 完成 | — |
| **v0.4.0** | 安全与架构 | ⚠️ 部分完成 | AgentLoop✅、统一配置✅；安全沙箱❌、工具注册❌、Token 预算❌ |
| **v0.5.0** | 学习与个性化 | ⚠️ 部分完成 | 反馈持久化✅、可观测性✅；流式输出❌、ANN 索引❌ |
| **v0.6.0** | Agency | ⚠️ 部分完成 | 目标系统✅、自主工具使用✅；安全沙箱仍缺失 |
| **v1.0.0** | 成熟版本 | ❌ 未开始 | 神经网络替换、多用户 |

**当前进度约在 v0.6.0 部分完成阶段，总工程量约完成 55-60%。**
...
3. **Agency 安全沙箱缺失**。`AgentLoop`、`GoalManager`、自主工具使用已实现，但 `CapabilityGuard` 仅有框架——无路径沙箱、无网络域名白名单、无审计日志。主动行为已能触发，但安全约束不足。
...
6. **工具系统部分硬编码**。`ToolHandler` trait + MCP 动态注册已有，但内置工具仍需改 `engine.rs` 分发逻辑。

7. **流式输出缺失**。LLM 完全缓冲后才发送，长回复时用户等待时间长，损害"类人感"。

8. **语音管道未实现**。`mneme_voice` 已删除，STT/TTS 无任何实现。

9. **配置管理已改善但仍有限**。`mneme.toml` 统一配置 + `reload` 命令已有，但缺少 schema 校验。

10. **`mneme_browser` 已退役**。浏览器自动化已迁移至 MCP Playwright server，原阻塞问题不再存在。
...
### 8.5 叙事系统：McAdams 理论的优秀实现，叙事盲区已补全
...
白皮书 §4.5 定义的 **叙事盲区**（Narrative Blind Spot）已实现：`SomaticMarker::narrative_blind_spot()` 检测多维度中等偏离稳态（energy、stress、mood_bias 中 ≥2 个处于"不正常但不极端"区间），在 `format_for_prompt_opacity()` 中注入"你可能不清楚为什么会有这种感觉——不需要解释原因"提示，防止 agent 过度理性化。
...
这个概念在代码中**部分实现**。`ReadingToolHandler`（v1.0.0 #56）提供了"书籍阅读→LLM 反思→价值信号提取"的管道，但尚无系统化的课程调度机制，且当前状态偏置解释过程（$s_{slow}$ 过滤）仍未实现。这是白皮书最具原创性的想法之一，值得继续深化。
...
1. **边缘系统神经网络**（§3.1）：白皮书设想用 Burn/Candle 实现可在线学习的 $F$ 函数，当前已有 LTC ODE + 赫布学习的自研实现（ADR-016/017），但非外部 ML 框架。建议先做参数敏感性分析，再决定哪些映射值得用神经网络替换。

2. **形成性课程**（§5.1）：书籍阅读 → 主观诠释 → 价值更新。`ReadingToolHandler`（v1.0.0 #56）已实现"阅读文本 → LLM 生成读书笔记 → 提取价值信号 → 注入 FeedbackBuffer"的管道。尚缺系统化的课程调度（如每日自动选书、阅读进度追踪）。

3. **主观诠释循环**（§5.1）：同一输入经过不同 $s_{slow}$ 过滤后产生不同梯度信号。当前无"当前状态偏置解释过程"的机制。

> 注：叙事盲区（§4.5）已实现（`SomaticMarker::narrative_blind_spot()`）；反馈缓冲已持久化到 SQLite（`feedback_signals` 表，含 `consolidated` 标记）；元认知反思循环已完整闭环（`handle_metacognition()` → insights → FeedbackBuffer → SleepConsolidation）。
...
- **白皮书落地率**：23 个核心概念中 21 个已有代码实现（91%）→ **极高**
...
| **理论-代码映射** | 9/10 | 白皮书 23 个核心概念中 21 个已落地（91%），映射准确 |
| **代码质量** | 8.5/10 | 编译零警告，348+ 测试，proptest 覆盖边界，f32 total_cmp 全覆盖 |
| **功能完成度** | 6/10 | 核心管道+安全沙箱+Agency+学习管道已实现，形成性课程/行为评估待完善 |
| **可上线程度** | 4/10 | CapabilityGuard 3-tier 沙箱已实现，但缺少生产级监控和灰度发布 |
| **科学严谨性** | 7.5/10 | 理论和工程映射优秀，但缺少实验验证和行为评估框架 |
| **文档质量** | 9.5/10 | 白皮书 + 设计文档 + ROADMAP 三层文档异常详尽 |

**一句话总结**：Mneme 拥有一套在个人项目中极为罕见的、有严肃认知科学基础的仿生架构。白皮书（`temp.md`）定义了一个"数字有机体"的完整蓝图，从 Damasio 的躯体标记到 Dąbrowski 的积极分裂，每一个理论引用都在代码中找到了对应结构。目前已建成约 55-60% 的工程量——灵魂（架构白皮书 + 多尺度动力学 + 结构性 LLM 调制 + 反馈缓冲安全机制 + 叙事系统 + LTC 神经调制器）和骨架（安全沙箱 + Agency 循环 + 学习管道 + 阅读管道 + MCP 工具系统）均已到位，但肌肉（形成性课程调度、行为评估框架、生产级监控、多 Mneme 通信）还需要继续建设。
