...
| mneme_memory | 12 | ✅ 完善 |
| mneme_limbic | 7 | ✅ 良好 |
| mneme_expression | 7 | ✅ 良好 |
| mneme_perception | 4 | ✅ 良好 |
| mneme_os | 4 | ✅ 良好 |
| **mneme_reasoning** | **0** | ❌ **零测试** |
| **mneme_onebot** | **0** | ❌ 零测试 |
| **mneme_browser** | **0** | ❌ 零测试 |
...
**需要实现**:
- [ ] Mock `LlmClient`（返回预设响应，可注入错误）
- [ ] ReAct 循环单元测试：正常对话、工具调用、多轮工具、递归限制触发
- [ ] 工具分发测试：每种工具的成功/失败路径
- [ ] 历史管理测试：prune 逻辑、窗口滑动、边界条件
- [ ] 集成测试：完整的 perception → memory → reasoning → expression 管道
- [ ] OneBot 事件解析与发送测试
