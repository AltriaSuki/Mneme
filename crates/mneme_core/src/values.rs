...
pub use crate::state::{ValueNetwork, ValueEntry};
...
impl RuleBasedJudge {
    pub fn new() -> Self {
        ...
        // Honesty patterns
        value_patterns.insert("honesty".to_string(), ValuePatterns {
        ...
        // Kindness patterns
        value_patterns.insert("kindness".to_string(), ValuePatterns {
            support_keywords: vec![
                "帮助", "关心", "体贴", "善良", "温暖", "照顾"
            ].into_iter().map(String::from).collect(),
            violation_keywords: vec![
        ...
        value_patterns.insert("authenticity".to_string(), ValuePatterns {
            support_keywords: vec![
                "真正", "自我", "真心", "本色", "坦然"
            ].into_iter().map(String::from).collect(),
            violation_keywords: vec![
        ...
        // Autonomy patterns
        value_patterns.insert("autonomy".to_string(), ValuePatterns {
            support_keywords: vec![
                "选择", "自主", "决定", "独立", "自由"
            ].into_iter().map(String::from).collect(),
            violation_keywords: vec![
        ...
        // Growth patterns
        value_patterns.insert("growth".to_string(), ValuePatterns {
            support_keywords: vec![
                "学习", "进步", "尝试", "挑战", "改变"
            ].into_iter().map(String::from).collect(),
            violation_keywords: vec![
        ...
        // Connection patterns
        value_patterns.insert("connection".to_string(), ValuePatterns {
            support_keywords: vec![
                "分享", "倾听", "理解", "陪伴", "交流"
            ].into_iter().map(String::from).collect(),
            violation_keywords: vec![
        ...
        // Curiosity patterns
        value_patterns.insert("curiosity".to_string(), ValuePatterns {
            support_keywords: vec![
                "探索", "好奇", "发现", "为什么", "研究"
            ].into_iter().map(String::from).collect(),
            violation_keywords: vec![
        ...
        let conflict_pairs = vec![
            ("honesty".to_string(), "kindness".to_string(), "残忍的真相".to_string()),
            ("authenticity".to_string(), "connection".to_string(), "不被接受".to_string()),
            ("autonomy".to_string(), "connection".to_string(), "依赖".to_string()),
            ("growth".to_string(), "安全".to_string(), "舒适区".to_string()),
        ];
...
impl ValueJudge for RuleBasedJudge {
    fn evaluate(&self, situation: &Situation, values: &ValueNetwork) -> JudgmentResult {
        ...
        let explanation = if has_conflict {
            format!(
                ...
                conflicts.iter().map(|c| format!("{} vs {}", c.value_a, c.value_b)).collect::<Vec<_>>().join("; "),
                supported.iter().map(|v| v.value_name.clone()).collect::<Vec<_>>().join(", "),
                violated.iter().map(|v| v.value_name.clone()).collect::<Vec<_>>().join(", "),
            )
    ...
    fn detect_conflicts(&self, situation: &Situation, values: &ValueNetwork) -> Vec<ValueConflict> {
        ...
        for (value_a, value_b, context_keyword) in &self.conflict_pairs {
            ...
            let has_a = values.values.get(value_a).map(|e| e.weight > 0.3).unwrap_or(false);
            let has_b = values.values.get(value_b).map(|e| e.weight > 0.3).unwrap_or(false);
    ...
    fn resolve_conflict(&self, conflict: &ValueConflict, values: &ValueNetwork) -> String {
        ...
        let score_a = values.values.get(&conflict.value_a)
            .map(|e| e.weight * (1.0 + e.rigidity))
            .unwrap_or(0.0);
        let score_b = values.values.get(&conflict.value_b)
            .map(|e| e.weight * (1.0 + e.rigidity))
            .unwrap_or(0.0);
...
impl HierarchicalValueNetwork {
    ...
    pub fn with_defaults() -> Self {
        ...
        // Core values
        values.insert("honesty".to_string(), HierarchicalValue {
        ...
        values.insert("kindness".to_string(), HierarchicalValue {
        ...
        values.insert("authenticity".to_string(), HierarchicalValue {
        ...
        // Derived values
        values.insert("curiosity".to_string(), HierarchicalValue {
        ...
        values.insert("growth".to_string(), HierarchicalValue {
        ...
        values.insert("connection".to_string(), HierarchicalValue {
    ...
    pub fn propagate_weights(&mut self) {
        ...
        let parent_weights: HashMap<String, f32> = self.values.iter()
            .map(|(k, v)| (k.clone(), v.weight))
            .collect();
        
        for value in self.values.values_mut() {
            if !value.parents.is_empty() {
                let parent_avg: f32 = value.parents.iter()
                    .filter_map(|p| parent_weights.get(p))
                    .sum::<f32>() / value.parents.len().max(1) as f32;
...
mod tests {
    ...
    fn test_rule_based_judge_violation() {
        ...
        assert!(result.violated_values.iter().any(|v| v.value_name == "honesty"));
