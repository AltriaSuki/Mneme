...
impl NarrativeWeaver {
    pub fn new() -> Self {
        ...
        theme_keywords.insert(
            "growth".to_string(),
            vec!["学习", "进步", "理解", "发现", "原来", "明白"]
                .into_iter()
                .map(String::from)
                .collect(),
        );
    ...
    pub fn weave_chapter(
        ...
        let period_start = episodes.iter().map(|e| e.timestamp).min()?;
        let period_end = episodes.iter().map(|e| e.timestamp).max()?;
        ...
        let emotional_tone =
            episodes.iter().map(|e| e.emotional_valence).sum::<f32>() / episodes.len() as f32;
    ...
    fn detect_themes(&self, episodes: &[EpisodeDigest]) -> Vec<String> {
        ...
        let mut themes: Vec<_> = theme_scores
            .into_iter()
    ...
    fn extract_people(&self, episodes: &[EpisodeDigest]) -> Vec<String> {
        let mut people: Vec<String> = episodes.iter().map(|e| e.author.clone()).collect();
    ...
    fn detect_turning_points(&self, episodes: &[EpisodeDigest]) -> Vec<TurningPoint> {
        ...
        for i in window_size..episodes.len() {
            let prev_avg = episodes[i - window_size..i]
                .iter()
    ...
    fn generate_narrative_content(
        ...
        if !themes.is_empty() {
            content.push_str(&format!("这段时间的主题是{}。", themes.join("、")));
        }

        // Episode count
        content.push_str(&format!("共有{}次互动记录。", episodes.len()));
        ...
        // Emotional summary
        let positive = episodes
            .iter()
            .filter(|e| e.emotional_valence > 0.2)
            .count();
        let negative = episodes
            .iter()
            .filter(|e| e.emotional_valence < -0.2)
            .count();
    ...
    pub fn detect_crisis(
        &self,
        recent_episodes: &[EpisodeDigest],
        current_narrative_bias: f32,
    ) -> Option<CrisisEvent> {
        ...
        let recent_avg = recent_episodes
            .iter()
            .map(|e| e.emotional_valence)
            .sum::<f32>()
            / recent_episodes.len() as f32;
        ...
        let conflict_count = recent_episodes
            .iter()
...
mod tests {
    ...
    fn test_crisis_detection() {
        ...
        let episodes: Vec<EpisodeDigest> = (0..5)
            .map(|i| EpisodeDigest {
