...
use mneme_memory::{
    Goal, GoalManager, GoalStatus, GoalType, LifecycleState, OrganismConfig, OrganismCoordinator,
    RuleContext, RuleEngine, RuleTrigger, SignalType, SqliteMemory,
};
use std::sync::Arc;

async fn setup_coordinator(
    dir: &tempfile::TempDir,
) -> (Arc<OrganismCoordinator>, Arc<SqliteMemory>) {
    ...
    let coord = Arc::new(
        OrganismCoordinator::with_persistence(limbic, config, db.clone())
            .await
            .unwrap(),
    );
    (coord, db)
...
async fn test_feedback_persistence_roundtrip() {
    ...
    // Phase 1: write feedback
    {
        ...
        let coord = OrganismCoordinator::with_persistence(limbic, config, db.clone())
            .await
            .unwrap();
...
async fn test_state_persistence_roundtrip() {
    ...
    // Phase 1: modify state and save
    {
        ...
        let limbic = Arc::new(LimbicSystem::new());
        let config = OrganismConfig::default();
        let coord = OrganismCoordinator::with_persistence(limbic, config, db)
            .await
            .unwrap();
    ...
    // Phase 2: reload and verify state
    {
        ...
        let state = state_arc.read().await;
        assert!(
            (state.fast.energy - 0.42).abs() < 0.01,
            "Energy should persist"
        );
        assert!(
            (state.medium.mood_bias - (-0.3)).abs() < 0.01,
            "Mood bias should persist"
        );
    }
}
...
async fn test_rule_engine_loads() {
    ...
    assert!(
        coord.rule_engine().is_some(),
        "Rule engine should be loaded"
    );
}
...
async fn test_goal_manager_roundtrip() {
    ...
    assert!(
        goals.is_empty(),
        "Completed goal should not appear in active list"
    );
}
...
async fn test_rule_engine_evaluate_and_persist() {
    ...
    let fired_names: Vec<&str> = results
        .iter()
        .filter_map(|(id, _)| {
            engine
                .rules()
        ...
        .collect();
    assert!(
        fired_names.contains(&"low_energy_silence"),
        "low_energy_silence should fire when energy < 0.2"
    );
    ...
    let rule = engine2
        .rules()
        .iter()
        .find(|r| r.name == "low_energy_silence")
        .unwrap();
    assert_eq!(
        rule.last_fired,
        Some(2000000),
